...
- **`gj`/`gk` now preserve the goal column correctly in Obsidian popout windows ("Open in new window"):** previously, crossing a table row or entering a table with `gj`/`gk` in a popout could reset it (landing at the start or end of the line instead), because the correction step's own deferred read ran too early relative to the freshly-created table cell's own inner view. Fixed by lengthening that wait to match the two-frame delay already used elsewhere in this codebase for the same class of Obsidian-internal reconciliation. Also switched every `requestAnimationFrame` call in this plugin from the bare `window` global to Obsidian's own popout-aware `activeWindow`, so each deferred repaint-dependent read stays synced to whichever window is actually being edited — the bare global always resolves to the *main* window's own repaint cycle regardless of which window is actually focused, which wasn't the cause of the `gj`/`gk` bug above but is the same class of issue. `setTimeout`/`setInterval` calls stay on the bare `window` global instead, per Obsidian's own `eslint-plugin-obsidianmd` guidance — a timer scheduled on `activeWindow` is destroyed if that popout closes before it fires, since plugin code always runs in the main window's own realm.
