...
- **Vim: bare `Space` no longer inserts a literal space after turning Table structure and Table navigation both off, without restarting:** these two features share one internal guard that stops an unmatched `<Space>` leader attempt from leaking through as a real keystroke, since Space's own native vim.js binding (removed the moment either feature is first turned on, to make room for the `<Space>t...` leader) can't be restored without an app reload. That guard was being torn down as soon as both features reported "off," even though Space's native binding was still permanently gone — so leaving it in this state (turning on just one of the two features, even briefly, was already enough) let a stray `Space` insert a literal space again. The guard now stays installed for the rest of the session once Space's native binding has actually been removed, matching the already-accepted "bare Space does nothing" behavior from while the features were still on, instead of regressing to the literal-space leak.
- **Kill region no longer eats the following newline when a selection starts at a Live Preview table cell's own start:** previously, this silently also consumed the next sub-line's leading newline (removing the resulting blank sub-line) instead of removing exactly what was selected. Real Emacs's own `kill-region` never does this — it removes exactly the selection, nothing more. Kill region's Live Preview path is now a plain selection delete, matching how it already behaved outside tables and in Source Mode.
- **Word right/left and Uppercase/Lowercase/Capitalize word now respect the Cross-row navigation setting inside tables, matching LEFT/RIGHT/HOME/END:** previously these commands always crossed to the next/previous table row, or exited the table entirely from the header row's left edge or the last row's right edge, even with Cross-row navigation turned off — they're now stopped at the row's own boundary in that case, just like every other table-aware command already was. Vim's own `w`/`b`/`e` are unaffected — this setting isn't exposed on the Vim tab, and real Vim's word motions have no notion of stopping at a row boundary to begin with.
...
- **`gj`/`gk` now preserve the goal column correctly in Obsidian popout windows ("Open in new window"):** previously, crossing a table row or entering a table with `gj`/`gk` in a popout could reset it (landing at the start or end of the line instead), because the correction step's own deferred read ran too early relative to the freshly-created table cell's own inner view. Fixed by lengthening that wait to match the two-frame delay already used elsewhere in this codebase for the same class of Obsidian-internal reconciliation. Also switched every deferred `setTimeout`/`requestAnimationFrame` call in this plugin from the bare `window` global to Obsidian's own popout-aware `activeWindow`, so each one stays synced to whichever window is actually being edited — the bare global always resolves to the *main* window regardless of which window is actually focused, which wasn't the cause of the `gj`/`gk` bug above but is the same class of issue.
- **`j`/`k` row-crossing and `w`/`b`/`e` cell-crossing no longer leave the cursor invisible in Obsidian popout windows:** previously, crossing a table row boundary with `j`/`k`, or a cell/row boundary with `w`/`b`/`e`, could land the cursor at a logically-correct position that simply never rendered on screen in a popout, until some unrelated repaint (e.g. switching desktops/spaces) revealed it was there all along. Fixed by adding a real layout measurement (`coordsAtPos`/`posAtCoords`) on the freshly-created table cell's own inner view immediately before re-dispatching the cursor position — the same combination `gj`/`gk`'s own correction step already relied on, which is why that motion never showed this symptom in the first place.
- **`gj`/`gk`'s goal column no longer drifts rightward when repeatedly crossing the same table row boundary:** previously, alternating `j`/`gk` (or any repeated `gk`/`gj` crossing back into the same row) across a row boundary shifted the remembered column a little further right on each cycle, since the crossing's own correction step was feeding its *raw-markdown* landing position (which includes the cell's own `| ` row prefix) back in as the next goal column, instead of the position relative to the cell's own content. Present since `gj`/`gk`'s table support first shipped in 0.8.0.
- **Kill Line no longer swallows an extra blank line inside a table cell:** previously, killing forward into a newline whose *next* sub-line was itself blank consumed both newlines in one keypress instead of one, because the Smart Join trim calculation was searching all the way to the cell's own end instead of stopping at that next sub-line's own boundary — a leading blank line was misread as skippable whitespace. Present since Kill Line's table-cell support first shipped in 0.6.0.
- **Emacs's `BOTTOM` (Cursor bottom) now lands at the true end of a table's rightmost cell, not one character short:** previously, when the last line of the note was part of a table, `BOTTOM` landed on the cell's second-to-last character instead of its actual content end (matching how it already landed at the real end of the line when the note didn't end in a table). Caused by a Vim-Normal-mode "can't rest past the last character" clamp — needed for `gg`/`G` and other Vim-adjacent table landings, since real Emacs has no such restriction — being applied unconditionally to every caller of the shared table-landing helper, including this one. Present since `BOTTOM`/`TOP` were split off from Vim's `gg`/`G` landing logic in 0.9.0.
- **Page Up/Down no longer occasionally stall then overshoot past a table in Obsidian popout windows:** previously, stepping through a table cell's own multi-line content there could freeze the measured scroll progress for many steps in a row (the popout's own layout measurement lagged behind the cursor's real position), then suddenly jump once it caught up — landing roughly twice as far down the page as intended, past where the table itself ends. A repeated stale reading is now treated as unmeasurable and falls back to a standard line-height estimate per step instead of silently accumulating no progress at all.
