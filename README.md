...
- **Entering a table from plain text always lands in the leftmost cell:** `gj`/`gk` moving from a plain-text line into an adjacent table row lands in that row's leftmost cell, regardless of which column the cursor was actually in — unlike row-to-row crossing *within* a table, which does preserve column. Obsidian's Live Preview table widget gives the outer editor no per-character position information for an unfocused table row, so there's no way to resolve which cell a given column falls under before landing in one.
...
- **Entering a table from plain text always lands in the leftmost cell:** Cursor UP/DOWN moving from a plain-text line into an adjacent table row lands in that row's leftmost cell, regardless of which column the cursor was actually in — unlike row-to-row crossing *within* a table, which does preserve column. Obsidian's Live Preview table widget gives the outer editor no per-character position information for an unfocused table row, so there's no way to resolve which cell a given column falls under before landing in one.
