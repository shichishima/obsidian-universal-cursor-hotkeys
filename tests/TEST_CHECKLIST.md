...
Legend: ✅ unit-testable · ⚙️ testable with editor mock · 🚫 requires Obsidian runtime

### Constants

| Symbol | Testable | Notes |
|--------|----------|-------|
...
- [ ] Row with multiple pipes `| a | b | c |` — returns correct indices
- [ ] Escaped pipe `\|` is excluded
- [ ] Line with no pipes — returns `[]`
...
- [ ] Valid index — returns correct end position
...
- [ ] `| a | b | c |` — returns `2`
- [ ] Single cell `| a |` — returns `0`

---

## `getCellIndex(line, ch)`

- [ ] ch in first cell — returns `0`
- [ ] ch in second cell — returns `1`
- [ ] ch before any pipe — returns `0` (clamped)

---

## `getBeginningOfLinePosition(line, ch)`

- [ ] Heading `## hello`, ch past prefix — returns index after `## `
- [ ] Heading in unordered list `- ## hello` — returns index after `- ## `
- [ ] Ordered list `1. item` — returns index after `1. `
- [ ] Unordered list `- item` — returns index after `- `
- [ ] Task list `- [ ] item` — returns index after `- [ ] `
- [ ] Blockquote `> text` — returns index after `> `
- [ ] Footnote `[^1]: note` — returns index after `[^1]: `
- [ ] Plain text — returns `0`
- [ ] Already at smart-home position — returns same position (no infinite toggle)

---

## `getInCellLineInfo(line, ch)`

- [ ] No `<br>` — `lineType: 'single'`, correct start/end
- [ ] One `<br>`: ch in first segment — `lineType: 'first'`
- [ ] One `<br>`: ch in last segment — `lineType: 'last'`
- [ ] Two `<br>`: ch in middle segment — `lineType: 'middle'`
- [ ] ch inside a `<br>` tag — assigned to preceding segment
- [ ] Spaces-only segment — `isEmpty: true`
...
- [ ] `prevLineInTable: false` — returns `-1`
- [ ] `prevLineInTable: true`, regular row — returns `currentLine - 1`
- [ ] `prevLineInTable: true`, delimiter row (`| --- |`) — returns `currentLine - 2`
- [ ] `prevLineInTable: true`, spaces-only `|     |` — treated as regular row, **not** delimiter (regression)

### `computeNextRowLine(currentLine, nextLineInTable, nextLineText, lineAfterNextInTable)`

- [ ] `nextLineInTable: false` — returns `-1`
- [ ] `nextLineInTable: true`, regular row — returns `currentLine + 1`
- [ ] `nextLineInTable: true`, delimiter row, `lineAfterNextInTable: true` — returns `currentLine + 2`
- [ ] `nextLineInTable: true`, delimiter row, `lineAfterNextInTable: false` (header-only table) — returns `-1`
- [ ] `nextLineInTable: true`, spaces-only `|     |` — treated as regular row, **not** delimiter (regression)
